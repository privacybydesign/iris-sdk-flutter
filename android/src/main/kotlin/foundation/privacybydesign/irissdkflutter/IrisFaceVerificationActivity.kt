package foundation.privacybydesign.irissdkflutter
...
import passportreader.PassportReader

/**
 * Hosts the Iris SDK's `startFaceVerification` flow.
 *
 * The SDK mounts its own camera preview and overlay into a [ViewGroup] it's
 * handed rather than exposing a headless per-frame API, so it needs a real
 * Activity to live in. This activity exists solely to give it one and to
 * translate its callback into an activity result the plugin can relay back
 * to Dart.
 *
 ...
 * draws.
 *
 * The vendor renamed the SDK from Iris to PassportReader; `passportreader.PassportReader`
 * is the current entry point and takes the place of the former `iris.Iris`.
 */
class IrisFaceVerificationActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        PassportReader(this).startFaceVerification(
            container,
            portrait,
            object : PassportReader.StartFaceVerificationHandler {
    ...
    companion object {
        ...
        fun sdkVersion(activity: Activity): String = PassportReader(activity).version()
    }
}
